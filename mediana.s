...
_start:
    ...
    mov x21, #0                 // Contador de posiciones
    ldr x1, =buffer             // Dirección del buffer en x1
...
parse_loop:
    ...
    cmp w2, #','                // Comparar con ','
    b.eq increment_position     // Si es ',', incrementar posición

    cmp w2, #0                  // Comparar con el fin de cadena '\0'
    beq parse_end               // Si es '\0', procesar el número
    strb w2, [x4], #1           // Guardar el byte en el buffer del número y avanzar
    b parse_loop                // Continuar el bucle

increment_position:
    add x21, x21, #1            // Incrementar contador de posiciones
    cmp x21, #6                 // Comparar con 6
    b.eq parse_number           // Si es la posición 6, procesar el número
...
parse_number:
    ...
    // Guardar el número en buffer_salida
    mov x20, x0                 // Guardar el número en x20
    b parse_end                 // Saltar al final del parseo

parse_end:
    // Convertir el número a cadena
    mov x0, x20                 // Pasa el número a x0
